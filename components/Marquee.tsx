...
export function Marquee() {
  ...
  return <div ref={marquee} className={`marquee ${paused ? "is-paused" : ""}`}><p className="sr-only">Áreas de Creator Forge: {items.join(", ")}.</p><div className="marquee-track" aria-hidden="true">{[...items, ...items].map((item, index) => <span key={`${item}-${index}`}>{item}<b>✦</b></span>)}</div><button className="marquee-toggle" type="button" aria-pressed={manualPaused} aria-label={reduced ? "El movimiento se redujo según tus preferencias" : manualPaused ? "Reanudar movimiento" : "Pausar movimiento"} disabled={reduced} onClick={toggle}>{manualPaused ? <Play aria-hidden="true" /> : <Pause aria-hidden="true" />}<span>{reduced ? "Movimiento reducido" : manualPaused ? "Reanudar" : "Pausar"}</span></button></div>;
}
